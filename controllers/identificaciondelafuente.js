const Identificaciondelafuente = require('../models/identificaciondelafuente');
const {ObjectId} = require('mongodb')

async function postIdentificaciondelafuente(req, res) {
    try {
        ...
        const response = await Identificaciondelafuente.create(arrayData);

        if (!response) return res.status(400).send({msg: "Error al guardar datos postIdentificaciondelafuente", status: false});
        return res.status(201).send(response[0]);
    } catch(error) {
        if (error.code === 11000) return res.status(503).send({msg: "Error server Identificaciondelafuente existe", status: false});

        return res.status(503).send({msg: "Error server postIdentificaciondelafuente", status: false});
    }
